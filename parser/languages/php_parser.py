...
def parse_php(tree, source_code, filename, symbol_table):
    ...
    for node in traverse(root):
        ...
        if node.type == 'class_declaration':
            ...
            if nm:
                cls_id = f'php::{filename}::{text(nm)}'
                nodes.append(Node(cls_id, 'CLASS', 'PHP', filename, node.start_point[0]+1, node.end_point[0]+1))
                symbol_table.add_definition('PHP', cls_id, 'CLASS', filename, curr_owner, node.start_point[0]+1, node.end_point[0]+1, body_text=text(node))
            ...
            if nm:
                f_nm = text(nm)
                fn_id = f'{curr_owner}::{f_nm}()' if curr_owner else f'php::{filename}::{f_nm}()'
                nodes.append(Node(fn_id, 'METHOD' if curr_owner else 'FUNCTION', 'PHP', filename, node.start_point[0]+1, node.end_point[0]+1))
                symbol_table.add_definition('PHP', fn_id, 'METHOD' if curr_owner else 'FUNCTION', filename, curr_owner, node.start_point[0]+1, node.end_point[0]+1, body_text=text(node))
