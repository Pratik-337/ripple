...
def parse_kotlin(tree, source_code, filename, symbol_table):
    ...
    for node in traverse(root):
        ...
        if node.type in {'class_declaration', 'object_declaration', 'interface_declaration', 'companion_object'}:
            ...
            symbol_table.add_definition('KOTLIN', fqn, kind, filename, curr_owner, node.start_point[0]+1, node.end_point[0]+1, body_text=text(node))
            
            # Inheritance
            delegation = node.child_by_field_name('delegation_specifier')
            if delegation:
                for spec in traverse(delegation):
            ...
            if body:
                for sub in traverse(body):
                    if sub.type == 'call_expression':
                        fexpr = sub.child_by_field_name('navigation_expression') or sub.child_by_field_name('function')
                        if fexpr:
