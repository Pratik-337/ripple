...
def validate_impact(changed_code, candidate_code, change_type="LOGIC_CHANGE"):
    pair_id = hashlib.md5((changed_code + candidate_code + str(change_type)).encode()).hexdigest()
    cache = get_cache()
    if pair_id in cache: return cache[pair_id]

    prompt = f"""
    [ROLE] Architect
    [CONTEXT] Change Type: {change_type}
    [CODE A (Changed)]
    {changed_code}
    [CODE B (Target)]
    {candidate_code}
    [TASK] If change in A impacts B, explain.
    [SCHEMA]
    ...
    return {"is_impacted": False, "reason": "Structural only (AI Fallback)", "suggested_fix": None, "confidence": 0.0}
