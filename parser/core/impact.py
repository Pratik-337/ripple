...
def propagate_impact(graph, start_node_id, max_depth=5):
    ...
    while queue:
        ...
        # Don't overwrite existing behavioral impact with structural impact
        if curr_id in impacted_nodes and impacted_nodes[curr_id]['via'] != 'CONTAINS' and via == 'CONTAINS':
            continue

        impacted_nodes[curr_id] = {'depth': depth, 'via': via}
        if depth < max_depth:
            for caller_id, rel_type in rev_adj.get(curr_id, []):
                # Always prioritize CALLS over CONTAINS in queue if needed
